...
def iabIdGrabber(x):
    try:
        for key, value in iabcats.items():
            if value == x:
                return key
                break
    except:
...
fem = pd.read_excel("femenine50000.xlsx", engine='openpyxl',
                    sheet_name='Sheet1', header=0, names=['id', 'interest'])
mas = pd.read_excel("masculine5000.xlsx", engine='openpyxl',
                    sheet_name='Sheet1', header=0, names=['id', 'interest'])
ignaxioDf = pd.concat([mas, fem])
dfSplitted = np.array_split(ignaxioDf["interest"], 500)
...
iabcats2 = {}
for i in iabcats:
    iabcats2[i.replace("-", "")] = iabcats[i]

tir1 = {}
for i in range(1, 27):
    if i not in [24, 25]:
        tir1[f"IAB{i}"] = iabcats[f"IAB{i}"]
t1CatLst = ",".join(tir1.values())


def dfChatGetter(inerestLsts):
    inters = "\n".join([str(i) for i in inerestLsts])
    question = f" map topics in list1 to the topics in list2 writing the results in csv format,\nlist2={inters}list2= {t1CatLst} e.g. Toyota , Automotive ;  Mario Bautista,Arts & Entertainment \n"
...
faulties = []
missings = [33]
for i in tqdm(missings):
    # for i in tqdm(range(501)):
    try:
        with open(f"./gptOuts4/part{i}.csv", "w") as file:

            file.write("interest,iabCat\n" +
                       dfChatGetter(dfSplitted[i].tolist()).replace(";", "\n"))
